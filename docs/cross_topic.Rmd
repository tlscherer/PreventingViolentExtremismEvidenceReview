...
datasheet_youth  <- readxl::read_excel(target_file, sheet="Youth") %>%
  ...
  dplyr::mutate_at(c("explanatory_variable_actionability", "explanatory_variable_proximity", "explained_variable_relevance", "data_transparency", "explanatory_power", "policy_and_practice_cogency", "research_framing", "reproducibility", "ethical_considerations"), as.numeric) %>% 
...
youth_evidence <- dplyr::bind_rows(
  ...
  youth_primary %>% tidyr::gather(x, value, c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>%
    dplyr::group_by(x) %>%
    dplyr::tally(value > 2) %>%
...
gender_evidence <- dplyr::bind_rows(
  ...
  gender_primary %>% tidyr::gather(x, value, c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>%
    dplyr::group_by(x) %>%
    dplyr::tally(value > 2) %>%
...
religion_evidence <- dplyr::bind_rows(
  ...
  religion_primary %>% tidyr::gather(x, value, c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>%
    dplyr::group_by(x) %>%
    dplyr::tally(value > 2) %>%
...
community_evidence <- dplyr::bind_rows(
  ...
  community_primary %>% tidyr::gather(x, value, c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>%
    dplyr::group_by(x) %>%
    dplyr::tally(value > 2) %>%
...
youth_long <- youth_primary %>% tidyr::pivot_longer(cols = c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>% 
  dplyr::mutate(value = as.character(value)) %>%
  dplyr::mutate(value = ifelse(is.na(value), "NA", value)) %>% 
  dplyr::mutate(value = factor(value, levels=c("NA", "1", "2", "3"))) %>% 
  dplyr::mutate(name = factor(name, levels=c("explanatory_actionability", "explanatory_proximity", "explained_relevance", "data_transparency", "explanatory_power", "program_discussion", "research_discussion", "reproducibility", "ethical_considerations"))) %>% 
  dplyr::mutate(name = dplyr::recode(name, 
                "explanatory_actionability" = "Explanatory Variable\nActionability",
                "explanatory_proximity" = "Explanatory Variable\nProximity", 
                "explained_relevance" = "Explained Variable\nRelevance", 
                "data_transparency" = "Data Transparency", 
                "explanatory_power" = "Explanatory Power", 
                "program_discussion" = "Program Discussion", 
...
gender_long <- gender_primary %>% tidyr::pivot_longer(cols = c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>% 
  dplyr::mutate(value = as.character(value)) %>%
  dplyr::mutate(value = ifelse(is.na(value), "NA", value)) %>% 
  dplyr::mutate(value = factor(value, levels=c("NA", "1", "2", "3"))) %>% 
  dplyr::mutate(name = factor(name, levels=c("explanatory_actionability", "explanatory_proximity", "explained_relevance", "data_transparency", "explanatory_power", "program_discussion", "research_discussion", "reproducibility", "ethical_considerations"))) %>% 
...
religion_long <- religion_primary %>% tidyr::pivot_longer(cols = c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>% 
  dplyr::mutate(value = as.character(value)) %>%
  dplyr::mutate(value = ifelse(is.na(value), "NA", value)) %>% 
  dplyr::mutate(value = factor(value, levels=c("NA", "1", "2", "3"))) %>% 
  dplyr::mutate(name = factor(name, levels=c("explanatory_actionability", "explanatory_proximity", "explained_relevance", "data_transparency", "explanatory_power", "program_discussion", "research_discussion", "reproducibility", "ethical_considerations"))) %>% 
...
community_long <- community_primary %>% tidyr::pivot_longer(cols = c(explanatory_actionability, explanatory_proximity, explained_relevance, data_transparency, explanatory_power, program_discussion, research_discussion, reproducibility, ethical_considerations)) %>% 
  dplyr::mutate(value = as.character(value)) %>%
  dplyr::mutate(value = ifelse(is.na(value), "NA", value)) %>% 
  dplyr::mutate(value = factor(value, levels=c("NA", "1", "2", "3"))) %>% 
  dplyr::mutate(name = factor(name, levels=c("explanatory_actionability", "explanatory_proximity", "explained_relevance", "data_transparency", "explanatory_power", "program_discussion", "research_discussion", "reproducibility", "ethical_considerations"))) %>% 
